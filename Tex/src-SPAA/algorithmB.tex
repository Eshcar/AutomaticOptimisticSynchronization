...
(When  \textbf{opt} is used to track the execution phase, \emph{track} is executed instead of locking whenever  \textbf{opt} is true).
...
prior to the validation as well as during \textit{isLockedByAnother}, because only read instructions are ordered.
...
Here, (i.e., in case  \textbf{opt} is false), 
before each unlock instruction \emph{\ttfamily x.unlock()} we insert the code \emph{\ttfamily x.version++} .
