...
To this end, the execution mode is tracked with a flag \textbf{opt}, initialized to true, indicating the optimistic phase. 
...
%			\State if {$\exists$<o,v>$\in$readSet such that o=obj and v!=ver} then
			...
			\If{$\langle$obj,$\rangle\in$readSet and v!=ver} \label{code:track:verifyVersion}
				\State return false
			\EndIf
