...
\caption{Small range: all threads execute either small range queries $[10,20]$
or a mix of insert and delete operations.}
\label{evaluation:results:skiplist:update}
\end{figure*}
...
\eshcar{small ranges: (1) all scans: results reflect expected order of the implementations ``best for queries''
(2) on the other extrem all updates
(3) in the middle mixed workload
(4) also tested range queries}
