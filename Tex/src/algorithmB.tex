...
We now explain our automatic technique to partition code $C$ to read-only and update phases $C_r$ and $C_w$, resp.
By slight abuse of terminology, we use the same notation ($C$, $C_r$, or $C_w$) both for the code section and 
...
An important feature of our transformation is that the shared state at the end of the validation phase
...
pessimistic version. This means that if the optimistic phases is unsuccessful for any reason, we can always
fall back on the pessimistic version. We use this property in two ways, as we now describe.

 First, we avoid livelock by limiting 
the number of restarts due to conflicts. The validation phase tracks the number of restarts in a thread-local variable, and 
if it exceeds a certain threshold, branches to $S'$, the matching clone for the beginning location 
of the operation, $S$. 

Second, this property offers us the liberty of allowing the optimistic implementation to
fail spuriously, even in the absence of conflicts, because it can always fallback on the safe pessimistic version
of the code. In the implementation in the next section, 
we take advantage of this liberty, and implement the \emph{lockedSet} and \emph{readSet} as constant 
size arrays. If too many items are read or locked, we also branch to $Sw$, 
the beginning of the operation's execution in $C_w$.  
