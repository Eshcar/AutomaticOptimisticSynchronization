...
(e.g, in Figure~\ref{figure:autoPartitioning:step2}, the matched location of line $5$ is line $5'$).
  \item [Step B:] 
Every location $l$ in $C$ that contains a write statement is replaced with the statement \emph{goto l'}.
This ensures that $C'$ will be executed just before the first write statement  is invoked.
In a similar manner, each location $l$ in $C$ which contains a \emph{return statement} (or any statement that exits from the operation)
is replaced with the statement \emph{goto l'}.
