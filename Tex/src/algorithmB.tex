...
(e.g, in Figure~\ref{figure:autoPartitioning:step2}, the matched location of line $5$ is line $5'$).
  \item [Step B:]
Every write instruction at location $l \in C$ is replaced with the instruction \emph{goto l'}.
This ensures that $C'$ will be executed just before the first write instruction is invoked.
In a similar manner, every \emph{return instruction} (or any instruction that exits from the operation) 
