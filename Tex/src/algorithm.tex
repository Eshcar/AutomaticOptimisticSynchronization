...
is replaced by removing the lock from the locked set.
...
The read set  tracks all objects loaded by the 
operation, in order to allow us to later validate that the collection of loaded values
reflects a consistent view of shared memory.  It maps each object (reference)  
to the object's local version at the time of the load. After an object is added to the read set, it is checked to be 
unlocked; if the object is locked, the operation restarts from the beginning.
...
Any validation failure causes the operation to restart.
