...
We compare the performance of our approach in terms of
throughput to fully pessimistic solutions applying fine-grain locking, solutions based on software transactional memory and hand-crafted state-of-the-art data structure
implementations.
...
\paragraph{Methodology} We use the micro-benchmark suite \textit{Synchrobench}~\cite{Gramoli2015}, configured as described below. 
...
$10^6$ keys and a warm-up of five seconds. The presented results are the average throughput over all trials.
...
\paragraph{Platform} All implementations are written in Java. We ran the experiments on a dedicated machine with
...
We employ textbook sequential implementations of an unbalanced internal binary
...
\autoTree and \autoTreap, respectively. Our manual transformation was applied line by line without requiring any understanding of the base code. To evaluate the overhead of tracking the \emph{lockedSet} we also applied the optimization described in Section \ref{sssec:alg-normal}, the resulting algorithms are denoted \optAutoTree and \optAutoTreap.   
We also applied Deuce~\cite{Deuce2010}, which is a Java implementation of the TL2 algorithm~\cite{DiceSS2006} to the sequential implementations. The resulting algorithms are denoted \stmTree and \stmTreap. We further compare our implementations to their hand-crafted state-of-the-art counterparts
listed in Table~\ref{table:hand-crafted}.
%\footnote{Unless described otherwise implementations are provided by Synchrobench.}. 

\Xomit{
We compare \autoTree to
\begin{description}
...
\begin{table}
\begin{tabular}{| l p{2.3in} |l  p{2.3in} |}
\hline 
  Unbalanced && Balanced  &\\  \hline 
  \textbf{\danaTree} &The locked-based unbalanced tree of Drachsler et al.~\cite{DrachslerVY2014} & \textbf{\danaAVL} &The locked-based relaxed balanced AVL tree of
				Drachsler et al.~\cite{DrachslerVY2014}  \\
  \textbf{\lockfreeTree} &The lock-free unbalanced tree of Ellen et al.~\cite{EllenFRB2010}  & \textbf{\bronson} &The locked based relaxed balanced AVL tree of Bronson et al.~\cite{BronsonCCO2010} \\
   && \textbf{\friendly} &The Contention-Friendly Tree of Crain et al.~\cite{CrainGR2013}  \\
   && \textbf{Skiplist} & Java lock free skiplist \\
   \hline 
\end{tabular}
\caption{Implementations of hand-crafted state-of-the-art data structures. The Implementation of \danaTree was provided by the authors, all other implementations provided by Synchrobenc.\label{table:hand-crafted}}
