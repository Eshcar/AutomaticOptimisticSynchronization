...
% Optimization
Although it only accesses thread-local data structures, lock tracking induces a certain overhead due to the need to search a lock in the \emph{lockedSet} when it is unlocked. (In our experiments presented below, in large data structures, this overhead slows the operation down by up to $25\%$). We can avoid this overhead in specific cases by relying on the structure of the transformed code. For example if the lock-based code is created from sequential code 
using domination locking~\cite{Gueta2011}, then at any given time in the execution, it holds locks on a well-defined set of nodes, which are the ones to which it also holds pointers. When applying our transformation to code generated by this scheme, we can optimize it to remove lock-tracking, and instead populate the \emph{lockedSet} with the appropriate locks immediately before executing the validation phase.
