...
public final class LockRemovalTree<K,V> implements CompositionalMap<K, V>{
    ...
    /*Node*/
    private final static class Node<K, V> extends SpinHeapReentrant {
    	/**
    	 * Default 
    	 */
    ...
    /*Thread Locals*/
    private final static class TreadLocals<K,V>{
    	final Thread self;
    ...
    */
	...
	public V get(Object key) {
		...
		while(true){
			err.clean();
			value = getImpl(k, threadLocals.self, threadLocals.readSet, err);
			if(!err.isSet()) break;
			retryCount ++;
			if(retryCount > LIMIT){
	...
	private V getImpl(Comparable<K> k, final Thread self, ReadSet<K,V> readSet, Error err) {
		readSet.clear(); 
		//lockSet.clear();
		try{
			long count = 0; 
