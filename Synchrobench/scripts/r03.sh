   ...
   for write in ${writes}; do
    for t in ${thread}; do
     for i in ${size}; do
	   ...
	   ${java} ${javaopt} -cp ${CP} ${MAINCLASS} -W ${warmup} -u ${write} -s ${snapshot} -d ${l} -t ${t} -i ${i} -r ${r} -f true --large-range true -b ${bench} 2>&1 >> ${out}
       done
     done
    done
   done
   ...
   for t in ${thread}; do
     for i in ${size}; do
       ...
       for stm in ${stms}; do
	     ...
	     ${java} ${javaopt} ${JVMARGS} -Dorg.deuce.transaction.contextClass=org.deuce.transaction.${stm}.Context -javaagent:${agent} -cp ${CP} ${BOOTARGS} ${MAINCLASS} -W ${warmup} -u ${write} -a ${writeall} -s ${snapshot} -d ${l} -t ${t} -i ${i} -r ${r} -f true --large-range true -b ${bench} -v 2>&1 >> ${out}
	 done
       done
     done
   done
